...
class ResidualAttentionModel_92_32input(nn.HybridBlock):
    ...
    def __init__(self, **kwargs):
        ...
        with self.name_scope():
            self.conv1 = nn.HybridSequential()
            with self.conv1.name_scope():
                self.conv1.add(nn.Conv2D(32, kernel_size=5, strides=1, padding=2, use_bias=False))
                self.conv1.add(nn.BatchNorm())
                self.conv1.add(nn.Activation('relu'))
            ...
            self.attention_module2 = AttentionModule_stage2_cifar(256)
            self.attention_module2_2 = AttentionModule_stage2_cifar(256)
            ...
            self.residual_block4 = ResidualBlock(1024, in_channels=512, stride=2)
            ...
            with self.mpool2.name_scope():
                self.mpool2.add(nn.BatchNorm())
                self.mpool2.add(nn.Activation('relu'))
                self.mpool2.add(nn.AvgPool2D(pool_size=4, strides=1))
            self.fc = nn.Conv2D(10, kernel_size=1)
...
class ResidualAttentionModel_92_32input_update(nn.HybridBlock):

    def __init__(self, **kwargs):
        super(ResidualAttentionModel_92_32input_update, self).__init__(**kwargs)
        """
        input size 32
        """
        with self.name_scope():
...
if __name__ == '__main__':
    cifar_net = ResidualAttentionModel_92_32input_update()
